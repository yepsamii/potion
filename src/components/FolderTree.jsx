...
import { 
  FiFolder, FiFolderOpen, FiMoreHorizontal, FiTrash2, 
  FiEdit3, FiPlus, FiChevronRight, FiChevronDown 
} from 'react-icons/fi'
import toast from 'react-hot-toast'

export default function FolderTree({ workspaceId, level = 0, parentId = null }) {
  ...
  const folders = useQuery(
    parentId 
      ? api.folders.getChildFolders
      : api.folders.getRootFolders,
      ...
      <div key={folder._id} className="select-none">
        ...
        >
          ...
          <span className="text-base">
            {isExpanded ? <FiFolderOpen className="w-4 h-4" /> : <FiFolder className="w-4 h-4" />}
          </span>
    ...
    return (
      <Link
        ...
        className={`flex items-center gap-2 px-2 py-1.5 text-sm rounded-md hover:bg-notion-gray-100 dark:hover:bg-notion-gray-800 cursor-pointer transition-colors ${
          location.pathname === `/document/${doc._id}` ? 'bg-notion-gray-200 dark:bg-notion-gray-700' : ''
        }`}
        style={{ paddingLeft }}
      >
  ...
  return (
    <div className="space-y-1">
      ...
      {level === 0 && documents?.filter(doc => !doc.parentId).map(renderDocument)}
